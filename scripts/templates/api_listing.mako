...
%if re.match(r"enum", obj['type']) or re.match(r"struct", obj['type']):
    * :ref:`${th.make_func_name_with_prefix('urPrint', obj['name'])}`
%endif
%endfor # obj in objects
%endfor # s in specs
...
%>\
    * :ref:`${th.make_func_name_with_prefix('urPrint', name)}`
%endfor
%endfor
## 'Extras' functions
    * :ref:`urPrintFunctionParams`
...
%if re.match(r"enum", obj['type']) or re.match(r"struct", obj['type']):
<%
    func_name = th.make_func_name_with_prefix('urPrint', obj['name'])
%>
...
<%
    name = th.make_pfncb_param_type(namespace, tags, obj)
    func_name = th.make_func_name_with_prefix('urPrint', name)
%>
${generate_api_doc(func_name)}
%endfor
%endfor
## 'Extras' functions
${generate_api_doc('urPrintFunctionParams')}
